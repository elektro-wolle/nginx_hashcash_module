...
.configure_nginx:
	cd nginx-1.21.4 && ./configure --add-dynamic-module="$(PWD)" --with-http_ssl_module --with-cc-opt="-I$(OPENSSL_HOME)/include -I$(LIBMEMCACHED_HOME)/include/" --with-ld-opt="-L$(OPENSSL_HOME)/lib" --with-debug
...
.build_module:
	cd nginx-1.21.4 && make -j8
	cp nginx-1.21.4/objs/ngx_http_hashcash_module.so bin/
