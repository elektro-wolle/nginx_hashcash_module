...
NGINX_HOME = $(PWD)/nginx-$(NGINX_VERSION)/
...
.configure_nginx:
	cd $(NGINX_HOME) && ./configure --add-dynamic-module="$(PWD)" --with-http_ssl_module --with-cc-opt="-I$(OPENSSL_HOME)/include -I$(LIBMEMCACHED_HOME)/include/" --with-ld-opt="-L$(OPENSSL_HOME)/lib" --with-debug
...
.build_module:
	cd $(NGINX_HOME) && make -j8
	cp $(NGINX_HOME) /objs/ngx_http_hashcash_module.so bin/
